...
const PATTERNS: Array<{ re: RegExp; mask: (m: string) => string }> = [
  ...
  {
    re: /\b(?:\+?92|0)3\d{2}[- ]?\d{7}\b/g,
    mask: () => "03** *** ****",
  },
  // Generic long phone numbers (international)
  {
  ...
  // Emails
  {
    ...
    mask: (_m, first: string, _rest: string, domain: string) =>
      `${first}***@${domain}`,
  },
  // OTP-ish 4-8 digit codes near the words otp / code / pin
  {
    re: /\b(otp|code|pin|passcode)[^\d]{0,10}(\d{4,8})\b/gi,
    mask: (_m, label: string) => `${label} ******`,
  },
  // Password: "password: xxxx"
  {
    re: /\b(password|pwd)\s*[:=]\s*\S+/gi,
    mask: () => "password: ******",
  },
];
...
export function maskSensitive(input: string): string {
  ...
  for (const { re, mask } of PATTERNS) {
    out = out.replace(re, mask as (substring: string, ...args: unknown[]) => string);
  }
  return out;
}
